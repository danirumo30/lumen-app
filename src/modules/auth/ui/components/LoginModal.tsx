...
export default function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPassword(value);
    if (!isLogin) setPasswordErrors(validatePasswordRealTime(value));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
      ...
      {/* Modal Content */}
      <div className="relative my-8 bg-zinc-950/90 border border-zinc-800 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all duration-300 animate-fade-in">
        {/* Header */}
        <div className="p-5 sm:p-8 border-b border-zinc-800/50 bg-zinc-900/30">
          ...
          <p className="text-sm text-zinc-400">
            {requiresVerification ? 'Confirma tu correo electrónico' : isLogin ? 'Accede a tu biblioteca' : 'Únete a Lumen'}
          </p>
        </div>
        ...
        {requiresVerification ? (
          ...
          <form onSubmit={handleForgotPassword} className="p-5 sm:p-8 space-y-4">
            <div className="text-center mb-4">
              <h3 className="text-lg font-semibold text-white">Restablecer contraseña</h3>
              <p className="text-sm text-zinc-400 mt-1">Ingresa tu email y te enviaremos un enlace.</p>
            </div>
            ...
            <div className="group">
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-zinc-900/50 border border-zinc-800 rounded-xl text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-indigo-500"
            ...
            <button
              ...
              className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl transition-all mt-4 disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <>
            <form onSubmit={handleSubmit} className="p-5 sm:p-8 space-y-4">
              {error && (
                <div className="p-3 sm:p-4 bg-red-900/20 border border-red-800/50 rounded-xl">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl transition-all mt-4 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isLoading ? (
                  <>
            ...
            {/* Footer */}
            <div className="px-5 sm:px-8 py-4 bg-zinc-900/30 border-t border-zinc-800/50">
              <div className="flex flex-col items-center space-y-2">
                ...
                <p className="text-center text-sm text-zinc-500">
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="ml-2 text-indigo-400 hover:text-indigo-300 font-medium transition-colors disabled:opacity-50"
                  >
